...
const MouUpdateForm = () => {
    ...
    const [formData, setFormData] = useState({ date: '', university: '', country: '', contactPerson: '', contactEmail: '', mouStatus: '', validityStatus: '' });
    ...
    const fetchItem = async () => {
        try {
            ...
            setFormData({ date: item.date ? new Date(item.date).toISOString().split('T')[0] : '', university: item.university || '', country: item.country || '', contactPerson: item.contactPerson || '', contactEmail: item.contactEmail || '', mouStatus: item.mouStatus || '', validityStatus: item.validityStatus || '' });
        } catch (error) {
