...
const ScholarForm = () => {
    ...
    const [formData, setFormData] = useState({ scholarName: '', country: '', department: '', fromDate: '', toDate: '' });
    ...
    const fetchItem = async () => {
        try {
            ...
            setFormData({ scholarName: item.scholarName || '', country: item.country || '', department: item.department || '', fromDate: item.fromDate ? new Date(item.fromDate).toISOString().split('T')[0] : '', toDate: item.toDate ? new Date(item.toDate).toISOString().split('T')[0] : '' });
        } catch (error) {
