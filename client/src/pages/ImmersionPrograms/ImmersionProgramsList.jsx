...
const ImmersionProgramsList = () => {
    ...
    const fetchItems = async () => {
        try {
            const response = await api.get('/immersion-programs');
            setItems(response.data.data || []);
        } catch (error) {
            toast.error('Error fetching immersion programs');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (reason) => {
        try {
            ...
            toast.success(isAdmin ? 'Program deleted successfully' : 'Delete request submitted for approval');
            fetchItems();
            window.dispatchEvent(new Event('pendingCountUpdated'));
        } catch (error) {
            toast.error(error.response?.data?.message || 'Error deleting program');
        }
    };

    const handleExportCSV = async () => {
        try {
            const response = await api.post('/immersion-programs/export-csv', {}, { responseType: 'blob' });
                ...
                <div className="card-body">
                    <div className="overflow-x-auto">
                        <table className="table table-zebra">
                            <thead><tr><th>Direction</th><th>University</th><th>Country</th><th>Pax</th><th>Dates</th><th>Status</th><th>Actions</th></tr></thead>
                            <tbody>
                                {items.length === 0 ? (
                                    <tr><td colSpan={7} className="text-center py-8">No programs found.</td></tr>
                                ) : (
                                    items.map((item) => (
                                        <tr key={item._id}>
                                            ...
                                            <td>{item.arrivalDate ? new Date(item.arrivalDate).toLocaleDateString() : '-'}</td>
                                            <td>
                                            ...
                                            <td>
                                                <div className="flex gap-2">
                                                    <Link to={`/immersion-programs/edit/${item._id}`} className={`btn btn-warning btn-sm ${item.status !== 'active' ? 'btn-disabled' : ''}`}><Edit size={16} /></Link>
